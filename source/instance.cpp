...
void Instance::WriteToBMP(const char *filename)
{
	int data_w, data_h;
	int image_w = 1000, image_h = 1000;
	float multiplier;

	data_w = min(GetDimensionX(), (int)sqrt(INT_MAX));
	data_h = min(GetDimensionY(), (int)sqrt(INT_MAX));
	...
	int x_distance = (int)(GetMiddleX() * (((float)image_w / (float)data_w) / 1.2));
	int y_distance = (int)(GetMiddleY() * (((float)image_h / (float)data_h) / 1.2));
